...
\edit{
\begin{theorem}
\cite{doi:10.1137/17M1111991} Consider a Hamiltonian system of the form \Cref{p3.eq:2.10.a,p3.eq:2.10.b,p3.eq:2.10.c} with a Hamiltonian $H\in C^{2}$  together with the reduced system \Cref{p3.eq:3.12.a,p3.eq:3.12.b,p3.eq:3.12.c}. Suppose that $(z_e,\phi_e,\theta_e)$ is a strict local minimum of $H_{\text{ex}}$. If we can find an open ball neighborhood $S$ of $(z_e,\phi_e,\theta_e)$ with respelct to the norm $\| \cdot \|_{\text{ex}}$ such that $\text{Range}(\mathbf A)\cap S \neq \emptyset$, then the reduced system \eqref{p1.eq:SyMo:4} remains bounded.
\end{theorem}
