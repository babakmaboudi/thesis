...
\begin{equation} \label{p1.eq:nommor.0}
	...
	\right.
\end{equation}
Here, $H: \mathcal Z \to \mathbb R$ is the Hamiltonian and $z\in \mathcal Z$ is the state vector. Suppose that $\mathcal Z$ is equipped with an inner-product $<\cdot,\cdot>_X : \mathcal Z \times \mathcal Z \to \mathbb R$ such that $<a,b>_X = a^TXb$, for all $a,b\in\mathcal Z$ and some symmetric and positive-definite matrix $X$. Note that $Z$ is not in general orthonormal with respect to $<,>_X$. As the matrix form of $\Omega$ is $\mathbb J_{2n}$, \Cref{theorem:2.12} implies that $Z$ is orthonormal with respect to the Euclidean inner product. Note that $z\in \mathcal Z$ implies that we can find $\alpha_i,\alpha'_i \in \mathbb R$, for $i=1,\dots,n$, such that $z = \alpha_i e_i + \alpha'_i f_i$. We have
\begin{equation}
	z = \alpha_i e_i + \alpha'_i f_i = \alpha_i X^{1/2} X^{-1/2}e_i + X^{1/2} X^{-1/2} \alpha'_i f_i.
\end{equation}
By defining basis vectors $\tilde e_i = X^{-1/2}e_i$, $\tilde e_i = X^{-1/2}f_i$, and basis $\tilde Z = \{ \tilde e_i , \tilde f_i \}_{i=1}^{n}$ with $\tilde z$ the state of a vector in this basis, we recover
\begin{equation}
...
\begin{equation}
	\Omega_{J_{2n}}(\tilde e_i,\tilde f_j) = \tilde e_i^T J_{2n} \tilde f_j = e_i^T X^{-1/2} X^{1/2} \mathbb J_{2n} X^{1/2} X^{-1/2} f_j = e_i^T \mathbb J_{2n} f_j = \Omega_{\mathbb J_{2n}} = \delta_{ij}.
\end{equation}
Here we used symplecticity of $Z$ with respect to $\mathbb J_{2n}$. Similarly we can verify that $\Omega_{J_{2n}}(\tilde e_i,\tilde e_j) = \Omega_{J_{2n}}(\tilde f_i,\tilde f_j) = 0$, for $i=1,\dots,n$. Using the state transformation $z = X^{1/2} \tilde z$, the Hamiltonian system \eqref{p1.eq:nommor.0} takes the form \footnote{substituting $z = X^{1/2}\tilde z$ in \eqref{p1.eq:nommor.0} yields
\[
\begin{aligned}
...
Note that the fundamental in \Cref{p1.theorem:1} we recover $(\tilde A^+)^T \neq \tilde A$ when $A$ is ortho-symplectic with respect to the Euclidean inner-product. However, when $\tilde A$ is orthonormal with respect to $<,>_X$ this property does not hold. This is particularly important when constructing a basis for nonlinear terms in \Cref{sec:normmor.3}.
