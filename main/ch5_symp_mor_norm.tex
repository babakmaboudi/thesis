...
As discussed previously, the evaluation of the nonlinear term in \eqref{p1.eq:nommor.0.11} retains a computational complexity proportional to the size of the full order system \eqref{p1.eq:nommor.0}. To overcome this, we take an approach similar to that in \Cref{sec:3.6}. \edit{Let $H(z) = H_1(z) + H_2(z)$ such that $\nabla_z H_1(z) = Lz$, for some $L\in \mathbb R^{2n\times 2n}$ and $\nabla_z H_2(z) = g(z)$, where $g:\mathbb R^{2n} \to \mathbb R^{2n}$ is a vector valued function. We use a $J_{2n}$-symplectic reduced basis $\tilde A$ to construct a reduced Hamiltonian system from \eqref{p1.eq:nommor.0.1} as
\begin{equation}
\begin{aligned}
