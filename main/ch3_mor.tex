...
\end{equation}
\edit{
Here $X^{1/2}$ is the matrix square root of $X$ defined as
\begin{equation}
	X^{1/2} = V \Lambda^{1/2} V^T,
\end{equation}
where $X=V \Lambda V^T$ is the eigenvalue decomposition of $X$.} Now if we define $\bar S = X^{1/2} S$ and $\bar W_k = X^{1/2} W_k$, we can rewrite (\ref{eq:3.20}) as 
