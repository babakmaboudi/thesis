\chapter{Symplectic Geometry and Hamiltonian systems} \label{chapter:2}
...
\edit{
This chapter is presented as a reminder of concepts in symplectic differential geometry and differential forms, as well as to introduce the notations used in the rest of this thesis. For a complete introduction to differential geometry we refer the reader to \cite{abraham1978foundations,hofer1995symplectic,marsden2013introduction}.
}

\section{Smooth Manifolds} \label{section:2.1}
\edit{
Let $\mathcal M$ be a topological Hausdorff space \cite{friedman1970foundations}. The pair $(x,U)$, where $U$ is an open subset of $\mathcal M$, $x:U\to V$ is a bijective map, and $V$ is an open subset of $\mathbb R^{m}$, is called a \emph{coordinate chart}. We call two coordinate charts $(x_1,U_1)$ and $(x_2,U_2)$ \emph{compatible}, if $U_1\cap U_2 \neq \emptyset$, $x_1(U_1\cap U_2)$ and $x_2(U_1\cap U_2)$ are open subsets of $\mathbb R^{m}$ and the mappings $x_2 \circ x_1^{-1}|_{x_1(U_1\cap U_2)}$ and $x_1 \circ x_2^{-1}|_{x_2(U_1\cap U_2)}$ are $C^{\infty}$.

$\mathcal M$ is called a differentiable manifold \cite{marsden2013introduction} if
\begin{enumerate} [label=(\alph*)]
\item $\mathcal M$ is covered by a collection of coordinate charts, i.e., every point $p$ of $\mathcal M$ belongs to at least one chart.
\item $\mathcal M$ can be written as a union of compatible charts. The union of such charts is referred to as an \emph{atlas}.
\end{enumerate}
Throughout this thesis, we assume that there exists a global coordinate chart $(x,U)$, i.e., an atlas that contains a single coordinate chart.
}
...
\edit{
A mapping $\gamma:\mathbb R \to \mathcal M$ is called a \emph{smooth curve} on $\mathcal M$ passing through $p\in \mathcal M$ if $\gamma(t) = p$, for some $t\in \mathbb R$, if $x \circ \gamma:\mathbb R \to \mathbb R^{m}$ is $C^{\infty}$.} Without loss of generality, we assume that $\gamma$ passes through $p$ at $t=0$. We define an equivalence relation between smooth curves that pass through $p$ as follows: $\gamma_1$ and $\gamma_2$, two smooth curves on $\mathcal M$, are equivalent if
...
\edit{It is well known that there is a natural vector space structure on $T_p\mathcal M$ \cite{abraham1978foundations,robbin2011introduction} by considering the directional derivative of differentiable real-valued functions defined on an open neighborhood of a point on $\mathcal M$.} Therefore, a basis for $T_p\mathcal M$ is often denoted as $\{ \partial /\partial x_i \}_{i=1}^{m}$ and a vector $v\in T_p\mathcal M$ can be written as 
\begin{equation} \label{eq:2.11}
	\sum_{i=1}^m v_i \frac{\partial}{\partial x_i},
\end{equation}
where $v_i\in \mathbb R$ for $i = 1,\dots,m$. Note that the symbol for the partial derivative is rather symbolic since the direction $x_i$ is not defined explicitly on $T_p\mathcal M$. \edit{To compute the coefficients $v_i$, for $i=1,\dots,m$, let $\gamma$ be a curve that is a representative of the equivalent class $v\in T_p \mathcal M$, the components of $v$ in a given chart $x$ are defined by the derivatives of the curve $x\circ \gamma:\mathbb R \to \mathbb R^{m}$, i.e.,
\[
	v_i = [\frac{d}{dt}(x\circ \gamma)]_i|_{t=0}.
\]
}To compute a tangent vector for a given chart $x$ and a given smooth function $f:\mathcal M \to \mathbb R$ one considers
...
The dual space to $T_p\mathcal M$ is denoted by $T_p^*\mathcal M$ and is referred to as the \emph{cotangent space}. The natural isomorphism between $T_p\mathcal M$ and $T_p^*\mathcal M$ implies that the cotangent space is also an $m$-dimensional linear vector space. Given $\{ \partial /\partial x_i \}$ as a basis for $T_p\mathcal M$, a dual basis for $T_p^*\mathcal M$ is a set of basis vectors $\{ dx_i \}_{i=1}^{m}$ that satisfy
...
\edit{
\begin{remark} \label{theorem:2.1}
...
\edit{
\begin{definition}
A continuous map $X:\mathcal M \to T\mathcal M$ is called a \emph{vector filed}, if $\pi \circ X = \text{id}_{\pi}$, where $\pi$ is the natural projection defined in \eqref{eq:2.2}, and $\text{id}_{\pi}$ is the identity map. 
\end{definition}
In other words, $X|_p \in T_{p}\mathcal M$, for any point $p\in \mathcal M$. Similarly, we can define a \emph{covector field}, as a continuous map $X^*:\mathcal M \to T^*\mathcal M$.
}
%\begin{definition}
...
\edit{
\begin{definition}
A vector field $X$ on $\mathcal M$ is \emph{complete} if for any point $p$, there exists an integral curve $c:\mathbb R \to \mathcal M$, such that $c(0) = p$, i.e., $c(t)$ is defined for $-\infty < t < \infty$.
\end{definition}
\begin{definition}
	The flow of a complete vector field $X$ is the unique map $\varphi:\mathbb R \times \mathcal M \to \mathcal M$, such that $\varphi_t(p) = \varphi(t,p)$ is an integral curve through $p$.
\end{definition}
}
%\begin{definition}
...
\]
be a smooth map, i.e., if $X_i$ and $X^*_j$, for $i=1,\dots,k$ and $j=1,\dots,l$, are $C^{\infty}$ vector fields and covector fields, respectively, the map $p\mapsto \Lambda_p(X_1,\dots,X_k,X^*_1,\dots,X^*_l)$ is a smooth map. $\Lambda_p$ is called a $(k,l)$-\emph{tensor}, if it is multilinear. A $(k,0)$-tensor $\Omega_p$ is called a \emph{differential} $k$-\emph{form}, if $\Omega_p$ is also skew-symmetric, i.e.,
\[
	\Omega_p(v_1,\dots,v_i,\dots,v_j, \dots, v_k) = - \Omega_p(v_1,\dots,v_j,\dots,v_i, \dots, v_k).
\]
\end{definition}
We may drop the superscript $p$ when the location of $p$ on $\mathcal M$ is not important.
}
...
%
%\begin{definition}
%A \emph{$(k,l)$-tensor} on a manifold $\mathcal M$ is a multilinear \edit{(not necessarily skew-symmetric)} function 
%\[
%	\Lambda_p:T_p^*\mathcal M \times \dots \times T_p^*\mathcal M \ (k \text{ times}) \times T_p\mathcal M \times \dots \times T_p\mathcal M \ (l \text{ times}) \to \mathbb R.
%\]
%\end{definition}
%\edit{When the point the exact location of $p$ is not important, we may drop the superscript $p$ and write $\Omega = \Omega_p$ and $\Lambda = \Lambda_p$.}
...
\begin{proof}
\edit{We remind the reader that a shorter proof of this theorem is available using Cartan's Magic Formula \cite{abraham1978foundations}. Here we provide a stand alone proof.} Let $(x,U)$ be a coordinate chart around $p\in \mathcal M$. Without loss of generality, we assume that $x(p)=0$ and that the image of $U$ under $x$ contains an open ball around the origin. We show that the pull-back of $\Omega$ to any point on this ball is exact.
...
\end{equation*}
Note that $T$ is bijective.
\begin{proposition} \label{theorem:2.11}
Let $(\mathcal Z,\Omega)$ be a $2n$-dimensional symplectic linear vector space. Furthermore, let $A = \{ e_1,\dots,e_n,f_1,\dots,f_n \}$ be a canonical basis for $\mathcal Z$ with respect to $\Omega$. Then
\begin{equation*}
	<u,v> = T^{-1}(u) (v),	
\end{equation*}
defines an inner product on $\mathcal Z$ and $A$ is orthonormal with respect to $<\cdot,\cdot>$.
\end{proposition}
\begin{proof}
It is straight forward to check that $<\cdot,\cdot>$ is symmetric, bilinear and positive-definite. Furthermore, we have
\[
	<e_i,e_j> = T^{-1}(e_i)(e_i) = -f_i^*(e_j) = -\Omega(f_i,e_j) = \Omega(e_i,f_j) = \delta_{ij}.
\]
for $i,j=1,\dots,n$. Similarly we can show
\[
	<f_i,f_j> = \delta_{ij}, \quad <e_i,f_j> = 0, \quad i,j=1,\dots,n.
\]
\end{proof}
}

%\begin{proposition} \label{theorem:2.11}
%Let $(\mathcal Z,\Omega)$ be a $2n$-dimensional symplectic linear vector space. Furthermore, let $A = \{ e_1,\dots,e_n,f_1,\dots,f_n \}$ be a canonical basis for $\mathcal Z$ with respect to $\Omega$. Then there exists an inner product $<,>:\mathcal Z\times \mathcal Z \to \mathbb R$, such that $A$ is orthonormal.
%\end{proposition}
%
%\begin{proof}
%Consider the linear mapping $\Omega^{\flat}:\mathcal Z \to \mathcal Z^*$ and let $e_i^*$ and $f_i^*$ be the image of $e_i$ and $f_i$ under $\Omega^{\flat}$, respectively for $i=1,\dots,n$. Non-degeneracy of $\Omega^{\flat}$ indicates that $e_i^*$ and $f_i^*$ are unique and non-zero, and that $A^*=\{e_1^*,\dots,e_n^*,f_1^*,\dots,f_n^*\}$ forms a basis for $\mathcal Z^*$. We construct a linear map $T:\mathcal Z^* \to \mathcal Z$ by prescribing its action on elements of $A^*$ as
...
Let $A=\{e_i,f_i\}_{i=1}^n$ be a canonical basis for $\mathcal Z$, the inner product constructed in \Cref{theorem:2.11} is the Euclidean inner product.
\end{corollary}

\begin{proof}
Let $u,v$ be a two vectors in $\mathcal Z$, and $v^*$ be the image of $v$ under $\Omega^{\flat}$. Let $\xi,\eta \in \mathbb R^{2n}$, be the expansion coefficients vector of $u$ and $v$ in the basis of $A$, respectively. Recall that $\Omega(u,v) = \xi^T \mathbb J_{2n} \eta$ in a canonical coordinate system.
		
We have
\begin{equation} \label{extra}
\begin{aligned}
	T^{-1}(u) = T^{-1}\left(\sum_i ( \alpha_i e_i + \alpha'_i e_i) \right) &= \sum_i \left( \alpha_i T^{-1}(e_i ) + \alpha'_i T^{-1}(f_i ) \right) \\
		&= \sum_i \left( -\alpha_i f_i^* + \alpha'_i e_i^* \right),
\end{aligned}
\end{equation}
where $\alpha_i$ and $\alpha_i'$ are the $i$th and the $(n+i)$th element of $\xi$, respectively. It follows
\begin{equation}
\begin{aligned}
	<u,v> = T^{-1}_u(v) &= \left(\sum_i \left( -\alpha_i f_i^* + \alpha'_i e_i^* \right) \right) \left( \sum_j \left( \beta_i e_i + \beta'_i f_i \right) \right) \\
	&=  \sum_{i} \alpha_i \beta_i + \alpha'_i \beta'_i = \xi^T \eta.
\end{aligned}
\end{equation}
Here, $\beta_i$ and $\beta_i'$ are the $i$th and the $(n+i)$th element of $\eta$, respectively. Furthermore, in the last step, we used the fact that $e^*_if_j = \Omega(e_i,f_j)= \delta_{ij}$, for $i,j=1,\dots,n$, and the skew-symmetry of $\Omega$. Furthermore, If we represent a co-vector as a transpose of a vector, \eqref{extra} implies that the expansion coefficients of $u^*$ is given by $\mathbb J_{2n} \xi$.
\end{proof}
}

%\begin{corollary} \label{theorem:2.12}
%When $\mathbb J_{2n}$ is the matrix form of $\Omega$, then $<,>$, constructed in \Cref{theorem:2.11}, is the Euclidean inner product. The canonical basis constructed with respect to $\mathbb J_{2n}$ is called an \emph{ortho-symplectic} basis.
%\end{corollary}

An inner product defined in \Cref{theorem:2.11} is especially useful since one can switch between computing the 2-form $\Omega$ and the inner production $<,>$, as needed. However, when dealing with a subspace of $\mathcal Z$, further structures are required to guarantee that such inner-product exists.
...
\edit{
\begin{definition}
Let $\varphi_t$ be the flow of a smooth vector field $X$ defined on $\mathbb R^{2n}$. The mapping $\Phi:\mathbb R \times \mathbb R^{2n} \to \mathbb R^{2n}$, for some initial condition $z_0\in \mathbb R^{2n}$, is a one step method of order $p$ if 
\begin{equation} \label{eq:2.23}
	\Phi_h(z_0) - \varphi_h(z_0) = \mathcal O(h^{p+1}).
\end{equation}
Furthermore, $\Phi$ is called a \emph{symplectic one step method} if $\varphi_t$ is a Hamiltonian flow and $\Phi$ is a symplectic transformation.
\end{definition}
}
%\begin{definition}
