...
\begin{theorem} Suppose that $X$ is a smooth vector field defined on a smooth manifold $\mathcal M$ with $\varphi$ the flow of $X$. Furthermore, suppose that $\Omega$ is a differential $k$-form. The following statements hold:
...
\begin{equation}
	\mathbf d \Omega_p(v_0,v_1,\dots,v_k) = \sum_i^k (-1)^i \nabla \Omega \cdot v_i (v_0,\dots,\bar{v_i},\dots,v_k), 
\end{equation}
where $\nabla \Omega \cdot v_i = \sum_j \sum_{i_1<\dots<i_k} v_i^j \partial \omega_{i_1,\dots,i_k}/\partial x_j$, $\bar{v_i}$ means that the slot for $v_i$ is empty, and $v_i^j$ is the $j$th component of $v_i$. A differential $k$-form $\Omega$ is called \emph{closed}, if $\mathbf d \Omega = 0$, and is called \emph{exact} there exist there is a differential $(k-1)$-form $\Gamma$ such that $\Omega = \mathbf d \Gamma$. The symmetry in partial derivatives implies that the exterior derivative of any differential form is closed, i.e., $\mathbf d^2 = 0$ for any differential $k$-form. However, no all differential forms are exact. We close this section by showing the sufficient condition for a differential form to be exact.

\begin{theorem}
(Poincar\'e Lemma) Let $\mathcal M$ be a smooth $n$-manifold with $\Omega$ a differential $k$-form, defined on $\mathcal M$. If $\Omega$ is closed then for any point $p\in \mathcal M$, there is a neighborhood $U$ of $p$ where $\Omega|_{U}$ is exact.
\end{theorem}

\begin{proof}
We construct an operator $\alpha:\Lambda_k\to \lambda_{k-1}$ such that  $\mathbf d \circ \alpha + \alpha \circ \mathbf d = id$. This yields the result since, $\mathbf d^2 \Omega = 0$ and $\mathbf d( \alpha (\Omega) ) = \Omega$. Let (x,U) be a coordinate chart in the neighborhood of $x$.

For the point $\mathbf X = (x_1,\dots,x_n)\in x(U)$, and vectors $v_1,\dots,v_{k-1}\in T_p \mathcal M$. We define $\alpha$ as
\begin{equation}
	\alpha_v(\Omega)(\mathbf x)(v_1,\dots,v_{k-1}) = \int_{0^1} t^{k-1} \Omega(tv)
\end{equation}
\end{proof}
...
\begin{definition}
	Let $\mathcal P$ be a smooth $m$-dimensional manifold and $\Omega$ be a closed, non-degenerate 2-form defined on $P$. The pair $(\mathcal P,\Omega)$ is called a \emph{symplectic manifold}.
\end{definition}
Note that the condition for $\Omega$ to be closed is indeed required to construct a well-defined symplectic manifold. In the following theorem, we will see that a closed form is required to locally approximate a symplectic manifold with a linear vector space.

\begin{theorem}

\end{theorem}

Furthermore, we know from \Cref{theorem:2.2} that given a coordinate chart $z$ on $\mathcal P$, $\Omega$ can be locally written as $\Omega = \sum_{i,j}^{m} \alpha_{i,j} dz_{i} dz_j$.
