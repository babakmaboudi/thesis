...
\begin{equation}
	p_{k+1} = p_k - h \cdot \nabla_q H(p_{k+1},q_k),\quad q_{k+1} = q_k + h \cdot \nabla_p H(p_{k+1},q_k),
\end{equation}
...
The symplectic Euler methods is s symplectic one step method of order 1.
\end{theorem}
\begin{proof}
...
\begin{equation*}
	...
	\end{pmatrix},
\end{equation*}
where $H_{yx}$ is an $n\times n$ matrix containing ${\partial^2 H }/{(\partial y_j \partial x_i)}$ in its $i$-th row and $j$-th column. It is straightforward to check that $(\Phi')^T \mathbb J_{2n} \Phi' = \mathbb J_{2n}$.
\end{proof}
