...
for i=1:numel(n_basis)
  for j=1:numel(n_reduction)
    ...
    % Compute invariants
    potential_energy = real(mesh.dx*sum(solution.pressure/(gamma-1)));
    kinetic_energy = real(mesh.dx*sum(0.5*solution.mod_mom.^2));
    potential_energy(isnan(potential_energy)) = 1000;
    kinetic_energy(isnan(kinetic_energy)) = 1000;
    potential_energy(abs(potential_energy)>100) = 100;
    kinetic_energy(abs(kinetic_energy)>100) = 100;
